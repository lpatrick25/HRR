            ...
            <div class="resort__details__content">
                <div class="row">
                    ...
                    <div class="col-lg-6 mb-4">
                        <img id="main-cottage-image" class="cottage-main-image" src="{{ asset($cottage->picture) }}"
                            alt="{{ $cottage->cottage_name }}">
                        <div class="d-flex mt-2">
                            @foreach ($cottage->pictures as $picture)
                                <img src="{{ asset($picture->picture) }}" class="cottage-thumbnail"
                                    onclick="changeCottageImage('{{ asset($picture->picture) }}')" alt="Cottage Thumbnail">
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="col-lg-6">
                        <div class="resort__details__text">
                            ...
                            <p>{{ $cottage->cottage_details ?? 'No details available' }}</p>

                            <div class="resort__details__widget">
                                <ul>
                                    <li><span>Type:</span> {{ $cottage->resortType->type_name }}</li>
                                    <li><span>Rate:</span> ₱{{ number_format($cottage->cottage_rate, 2) }}</li>
                                    <li><span>Capacity:</span> {{ $cottage->cottage_capacity }} Persons</li>
                                </ul>
                            </div>
                            ...
                            <div class="resort__details__btn">
                                <button type="button" class="follow-btn"
                                    ...
                                    <i class="fa fa-heart-o me-2"></i> Book Now <i class="fa fa-angle-right"></i>
                                </button>
                                <button type="button" class="follow-btn"
                    ...
                    @if (auth()->user())
                        ...
                        <div class="resort__details__form mt-5 p-4 border rounded bg-light">
                            ...
                            <form id="insertReviews">
                                <input type="hidden" id="resort_cottage_id" name="resort_cottage_id"
                                    value="{{ encrypt($cottage->id) }}">
...
@section('APP-SCRIPT')
    ...
    <script type="text/javascript">
        ...
        function changeCottageImage(imageUrl) {
            document.getElementById('main-cottage-image').src = imageUrl;
        }
        ...
        async function validateStep() {
            ...
            if (currentStepAdd === 2) {
                ...
                if (valid) {
                    try {
                        const response = await fetchAvailableCottage(bookingDate, {{ $cottage->id }});
                        if (!response.valid) {
        ...
        function fetchAvailableCottage(bookingDate, cottageID) {
            return new Promise((resolve, reject) => {
                $.ajax({
                    url: '{{ route('getCottageAvailability') }}',
                    method: 'GET',
                    data: {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response) {
                        ...
                        $('#bookingModal #booking_date').removeAttr('readonly');
        ...
        $(document).ready(function() {
            ...
            $('#insertReviews').submit(function(event) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '{{ route('homepage-insertCottageReviews') }}',
