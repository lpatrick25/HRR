...
@section('APP_CONTENT')
    <div class="row">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <div class="contact-client-img text-center" style="position: relative;">
                            <a href="#">
                                <img src="{{ $cottageInfo->picture ? asset($cottageInfo->picture) : asset('img/profile/1.jpg') }}"
                                    id="cottage-thumbnail" alt="Cottage Image"
                    ...
                    <div class="col-lg-6">
                        <div class="contact-client-content">
                            <h2><a href="#" style="text-decoration: none; color: inherit;">Name:
                                    {{ $cottageInfo->cottage_name }}</a></h2>
                            <p><i class="fa fa-map-marker"></i> <strong>Type:</strong>
                                {{ $cottageInfo->resortType->type_name }}
                            </p>
                        </div>

                        <div class="contact-client-address" style="margin-top: 10px;">
                            <p class="address-client-ct"><strong>Status:</strong>
                                {{ ucfirst($cottageInfo->cottage_status) }}</p>
                            <p class="address-client-ct"><strong>Rate:</strong>
                                {{ number_format($cottageInfo->cottage_rate, 2) }}
                                PHP</p>
                            <p class="address-client-ct"><strong>Capacity:</strong> {{ $cottageInfo->cottage_capacity }}
                                Person{{ $cottageInfo->cottage_capacity > 1 ? 's' : '' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-6">
            <div class="sparkline12-list shadow-reset">
                ...
                <div id="cottagePicturesContainer" class="sparkline12-graph clearfix">
                    @include('partials.cottage_pictures', ['pictures' => $cottageInfo->pictures])
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <form id="addPictureForm" class="modal-content">
                ...
                <div class="modal-body">
                    <input type="hidden" name="resort_cottage_id" id="resort_cottage_id" value="{{ $cottageInfo->id }}" />
                    <div class="row">
                        <div class="col-md-6">
    ...
    <div id="addCottagePictureModal" class="modal animated fadeIn" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <form id="addCottagePictureForm" class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">Add Cottage Picture
                </div>
                <div class="modal-body">
                    <input type="hidden" name="resort_cottage_id" id="resort_cottage_id"
                        value="{{ $cottageInfo->id }}" />
                    <div class="row">
                        <div class="col-md-6">
                            <div class="image-crop">
                                <img src="{{ asset('img/cropper/1.jpg') }}" alt="">
...
@section('APP_SCRIPT')
    <script type="text/javascript">
        ...
        function reloadCottagePictures() {
            $.ajax({
                method: 'GET',
                url: `/resortPictures/{{ $cottageInfo->id }}`, // Create this route in your controller
                dataType: 'html',
                success: function(response) {
                    $('#cottagePicturesContainer').html(response); // Replace image container
                },
                error: function() {
                    showErrorMessage('Failed to refresh cottage pictures.');
                }
            });
        }

        $(document).ready(function() {
            var cropperResortPicture = null;
            var cropperCottagePicture = null;
            ...
            $('#add-btn-two').click(function(event) {
                ...
                $('#addPictureModal').modal({
                    ...
                    if (!cropperResortPicture) {
                        cropperResortPicture = new Cropper(image, {
                            aspectRatio: 1.618,
                            viewMode: 1,
                    ...
                    if (cropperResortPicture) {
                        cropperResortPicture.destroy();
                        cropperResortPicture = null;
                    }
                });
            });

            // Open Add Cottage Picture Modal
            $('#add-btn-three').click(function(event) {
                event.preventDefault();
                $('#addCottagePictureModal').modal({
                    backdrop: 'static',
                    keyboard: false,
                    show: true
                }).on('shown.bs.modal', function() {
                    const image = document.querySelector(
                        '#addCottagePictureModal .image-crop > img');
                    if (!cropperCottagePicture) {
                        cropperCottagePicture = new Cropper(image, {
                            aspectRatio: 1.618,
                            viewMode: 1,
                            preview: '#addCottagePictureModal .img-preview',
                        });
                    }
                }).on('hidden.bs.modal', function() {
                    if (cropperCottagePicture) {
                        cropperCottagePicture.destroy();
                        cropperCottagePicture = null;
                    }
                });
            });

            // Image Upload for Resort Picture Modal
            $('#addPictureModal #inputImage').on('change', function(event) {
                ...
                reader.onload = function(e) {
                    if (cropperResortPicture) {
                        cropperResortPicture.replace(e.target.result);
                    }
                };
                reader.readAsDataURL(file);
            });

            // Image Upload for Cottage Picture Modal
            $('#addCottagePictureModal #inputImage').on('change', function(event) {
                const file = event.target.files[0];
                if (!file) return;
                ...
                reader.onload = function(e) {
                    if (cropperCottagePicture) {
                        cropperCottagePicture.replace(e.target.result);
                    }
                };
                reader.readAsDataURL(file);
            });
            ...
            $('#addPictureForm').submit(function(event) {
                ...
                cropperResortPicture.getCroppedCanvas().toBlob(function(blob) {
            ...
            // Submit Cottage Picture
            $('#addCottagePictureForm').submit(function(event) {
                event.preventDefault();
                ...
                cropperCottagePicture.getCroppedCanvas().toBlob(function(blob) {
                    const formData = new FormData($('#addCottagePictureForm')[0]);
                    formData.append('picture', blob, 'cropped_image.png');

                    $.ajax({
                        method: 'POST',
                        url: '/resortCottages/cottagePicture',
                        data: formData,
                        processData: false,
                        contentType: false,
                        dataType: 'JSON',
                        success: function(response) {
                            if (response.valid) {
                                $('#addCottagePictureModal').modal('hide');
                                $('#addCottagePictureForm')[0].reset();
                                $('#cottage-thumbnail').attr('src', '/' + response
                                    .cottage
            ...
            function getActiveCropper($button) {
                ...
                if ($modal.is('#addPictureModal')) {
                    return cropperResortPicture;
                } else if ($modal.is('#addCottagePictureModal')) {
                    return cropperCottagePicture;
                }
                return null;
            }
        });
